...
const RULES: Partial<Record<string, RuleDef>> = {
  ...
  mahjong:    { icon: "mahjong",   lines: ["Retirez les tuiles par paires identiques.", "Une tuile est jouable si elle est libre sur au moins un côté.", "Attention aux tuiles à bordure rouge : ce sont des pièges !", "Videz tout le plateau pour gagner !"] },
  ...
  illusions:  { icon: "eye",       lines: ["Observez l'image et répondez : votre œil dit-il vrai ?", "Du plus simple (Découverte) au plus surprenant (Expert), ou en mode Varié.", "Après chaque réponse, la preuve se révèle et explique le « truc »."] },
  route:      { icon: "car",       lines: ["Une plaque ou un panneau s'affiche : répondez à la question.", "Mode Plaques : numéros, préfectures et régions des départements.", "Mode Routes : autoroutes, ponts et grandes routes de France. Mode Varié : un mélange des deux !"] },
